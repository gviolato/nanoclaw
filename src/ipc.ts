...
export interface IpcDeps {
  ...
  sendFile?: (jid: string, hostFilePath: string, filename: string, comment?: string) => Promise<void>;
              ...
              if (data.type === 'message' && data.chatJid && data.text) {
                ...
                if (isMain || (targetGroup && targetGroup.folder === baseFolder)) {
                  if (deps.sendFile) {
                    ...
                    logger.info({ chatJid: data.chatJid, filename, sourceGroup }, 'IPC file upload sent');
                  } else {
                    logger.warn({ chatJid: data.chatJid }, 'upload_file IPC received but sendFile not supported by channel');
                  }
                } else {
                  logger.warn({ chatJid: data.chatJid, sourceGroup }, 'Unauthorized IPC upload_file attempt blocked');
                }
              }
